        ...
        <div class="row">
            ...
            <div class="col-lg-12">
                ...
                <table class="data-table table mb-0 tbl-server-info">
                    ...
                    <c:if test="${not empty listLockxu }">
                    	<c:forEach items="${listLockxu }" var="lockxu">
                    	 <tr>
                            <td>${lockxu.users.username }</td>
                            <td>${lockxu.users.name }</td>
                            <td>${lockxu.surplus }</td>
                            <td>
                                <div class="d-flex align-items-center list-action">
                                        <c:if test="${users.username ne pageContext.request.userPrincipal.name }">
                                    <a class="badge bg-warning mr-2" data-toggle="tooltip" data-placement="top" title="" data-original-title="Delete"
                                        href="<%=request.getContextPath()%>/admin/delete-lockpay?id=${lockxu.id }" ><i onclick="return confirm('Bạn thật sự muốn xóa ?')" class="ri-delete-bin-line mr-0"></i></a>
                                	</c:if>
                                </div>
                            </td>
                        </tr>
